...
const DEFAULT_COLUMNS = [
  ...
  { key: 'zkkid', label: 'ЗККИД', width: '120px' },
  { key: 'position', label: 'Позиция', width: '150px' },
  { key: 'owner', label: 'Владелец', width: '150px' },
  { key: 'brand', label: 'Марка', width: '120px' },
  { key: 'model', label: 'Модель', width: '150px' },
  { key: 'gosnumber', label: 'Гос. номер', width: '120px' },
  { key: 'serial', label: 'Серийный номер', width: '150px' },
  { key: 'hoznumber', label: 'Хоз. номер', width: '120px' },
  { key: 'year', label: 'Год', width: '80px' },
  { key: 'narabotka', label: 'Наработка', width: '120px' },
  { key: 'izmerenie_narabotka', label: 'Изм. наработка', width: '140px' },
  { key: 'condition', label: 'Состояние', width: '120px' },
  { key: 'kapital_remont', label: 'Капитальный ремонт', width: '150px' },
  { key: 'note', label: 'Примечание', width: '200px' },
  { key: 'Encumbrance', label: 'Обременение', width: '120px' },
  { key: 'inventory_number', label: 'Инвент. номер', width: '140px' },
  { key: 'width', label: 'Ширина', width: '100px' },
];

export default function AutoparkTable({ userId }) {
  ...
  useEffect(() => {
    async function loadPermissions() {
      ...
      if (result.success && result.role) {
        ...
        // Настраиваем колонки на основе прав доступа
        const visibleColumnKeys = result.role.permissions.visibleColumns || [];
        const editableColumnKeys = result.role.permissions.editableColumns || [];
        
        const configuredColumns = DEFAULT_COLUMNS.map(col => ({
          ...col,
          visible: visibleColumnKeys.includes(col.key),
          editable: editableColumnKeys.includes(col.key) && result.role.permissions.canEditAutopark
        }));
        
        setColumns(configuredColumns);
      } else {
        // Если нет роли, показываем все колонки как только для чтения
        ...
        setPermissions({
          canViewAutopark: true,
          canViewDetails: true,
          canEditAutopark: false,
          canCreateAutopark: false,
          canDeleteAutopark: false,
          canViewFiles: false,
  ...
  }, [page, limit, sortBy, sortOrder, loadingPermissions, permissions]);

  async function loadDocuments() {
    // Проверяем право на просмотр
    if (!permissions?.canViewAutopark) {
      setDocuments([]);
      setTotal(0);
      setLoading(false);
      return;
  ...
  async function addNewRow() {
    ...
    if (!permissions?.canCreateAutopark) {
      alert('У вас нет прав на создание записей');
      return;
    }
  ...
  async function deleteRow(rowId) {
    ...
    if (!permissions?.canDeleteAutopark) {
  ...
  // Если еще загружаются права
  if (loadingPermissions) {
    return (
      <div className="bg-white rounded-xl shadow-xl p-16 text-center">
        <div className="inline-block animate-spin rounded-full h-12 w-12 border-4 border-blue-600 border-t-transparent"></div>
        <p className="mt-4 text-gray-600 font-medium text-lg">Загрузка прав доступа...</p>
      </div>
    );
  }
  ...
  if (!permissions?.canViewAutopark) {
    return (
      <div className="bg-white rounded-xl shadow-xl p-16 text-center">
        ...
        <p className="text-gray-600">У вас нет прав для просмотра списка автопарка</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl shadow-xl overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-blue-500 to-blue-500 p-4">
        ...
        {/* Панель настройки колонок (только для администратора) */}
        {showColumnSettings && permissions?.canManageRoles && (
          <div className="mt-4 bg-white rounded-lg p-4 shadow-lg">
            <div className="flex justify-between items-center mb-3">
              <h3 className="font-bold text-gray-800 text-lg">Настройка отображения колонок (локально)</h3>
              <button
            ...
            <p className="text-sm text-gray-600 mb-3">
              Это временная настройка для вашей сессии. Для изменения прав доступа роли используйте раздел "Управление ролями".
            </p>
      ...
      {userRole && (
        <div className="bg-blue-50 border-b border-blue-200 px-6 py-3">
          <div className="flex items-center text-sm text-blue-800">
            ...
            <span>
              Ваша роль: <strong>{userRole.name}</strong>
                      ...
                      >
                        {isEditing ? (
                          ...
                          <div
                            ...
                            className={`px-3 py-2.5 h-full ${col.editable ? 'cursor-pointer hover:bg-yellow-50' : 'cursor-default'} transition-colors duration-150 ${
                              !cellValue ? 'text-gray-400 italic' : 'text-gray-800'
                            }`}
                          >
                            {cellValue || (col.editable ? 'Нажмите для ввода' : '-')}
                          </div>
                        )}
                      </td>
                    );
